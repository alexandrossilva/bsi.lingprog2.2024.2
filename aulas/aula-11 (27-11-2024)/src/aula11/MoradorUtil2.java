    ...
    public static void lerDadosCandidatos() {
    	...
    	for (int i = 0; i < qtd; i++) {
    		...
    		for (int j = 0; j < candidatos.size(); j++) {
    			Morador mTemp = candidatos.get(j);
    			
    			if (unidade == mTemp.getNumeroUnidade() || numeroVotacao == mTemp.getNumeroVotacao()) {
    				valido = false;
    				break;
    			}
    		}
    		...
    		if (!valido) {
        		System.out.println("Número de unidade e/ou de votação já cadastrado anteriormente!");    			
        		System.out.println("CANDIDADO A SER CADASTRADO NOVAMENTE!");    			
				i--;
    		}
    		else
	    		candidatos.add(m);
